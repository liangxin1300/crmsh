...
Feature: Verify usercase master survive when split-brain
  ...
  According to https://confluence.suse.com/x/SoaHFQ:
  1. Setup a two-nodes cluster
  2. Generate script to check whether this node is master
  3. Add a promotable clone resource and make sure hanode1 is master(use location)
  4. Setup qdevice with heuristics
  5. Use iptables command to simulate split-brain
  6. Check whether hanode1 has quorum, while hanode2 doesn't(return code is 2)
  ...
  Scenario: Master survive when split-brain
    ...
    And     Run "crm configure location loc1 promotable-1 100: hanode1" on "hanode1"
