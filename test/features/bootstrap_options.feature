@bootstrap
Feature: HA bootstrap process - options

  Scenario: Init whole cluster service on node "hanode1"
  ...
  @clean
  Scenario: Bind specific network interface
    Given   Cluster service is "stopped" on "local"
  ...
  @clean
  Scenario: Using multiple network interface
    Given   Cluster service is "stopped" on "local"
  ...
  @clean
  Scenario: Setup cluster name and virtual IP
    Given   Cluster service is "stopped" on "local"
