...
Feature: crmsh bootstrap sbd management
  ...
  Scenario: Configure diskless sbd(bsc#1181907)
    ...
    Then    Expected "WARNING: Diskless SBD requires cluster with three or more nodes." in stdout
    Then    Cluster service is "started" on "hanode1"
    And     Service "sbd" is "started" on "hanode1"
    When    Run "crm cluster join -c hanode1 -y" on "hanode2"
    Then    Expected "WARNING: Diskless SBD requires cluster with three or more nodes." in stdout
    ...
    Then    Expected "WARNING: Diskless SBD requires cluster with three or more nodes." not in stdout
