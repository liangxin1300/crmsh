...
class TestContext(unittest.TestCase):
    ...
    def test_str(self, mock_dumps):
        ...
        self.assertEqual(self.context.dumps(), "json str")
...
class TestRun(unittest.TestCase):
    ...
    def test_pick_compress_prog_return(self, mock_which):
        mock_which.return_value = True
        mock_ctx_inst = mock.Mock(compress_prog=None, compress_suffix=None)
        core.pick_compress_prog(mock_ctx_inst)
        self.assertEqual(mock_ctx_inst.compress_prog, "bzip2")
        self.assertEqual(mock_ctx_inst.compress_suffix, ".bz2")
        mock_which.assert_called_once_with("bzip2")

    @mock.patch('shutil.which')
    def test_pick_compress_prog(self, mock_which):
        mock_which.side_effect = [False, False, False]
        mock_ctx_inst = mock.Mock(compress_prog=None, compress_suffix=None)
        core.pick_compress_prog(mock_ctx_inst)
        self.assertEqual(mock_ctx_inst.compress_prog, "cat")
        self.assertEqual(mock_ctx_inst.compress_suffix, "")

    @mock.patch('crmsh.report.utils.get_timespan_str')
    @mock.patch('logging.Logger.info')
    def test_finalword(self, mock_info, mock_get_timespan):
        mock_ctx_inst = mock.Mock(dest_path="./crm_report-Tue-15-Aug-2023.tar.bz2")
    ...
    def test_setup_workdir_collector(self, mock_create_dir, mock_collector, mock_this_node, mock_mkdirp, mock_debug, mock_basename):
    ...
    def test_load_context_attributes_warning(self, mock_load, mock_isdir, mock_exists, mock_warn, mock_corosync_conf):
        ...
        mock_warn.assert_called_once_with(f"File {mock_corosync_conf.return_value} does not exist")
    ...
    @mock.patch('os.path.isdir')
    @mock.patch('crmsh.report.core.load_from_crmsh_config')
    def test_load_context_attributes(self, mock_load, mock_isdir, mock_exists, mock_corosync_get_value):
        mock_ctx_inst = mock.Mock(cib_dir="/var/lib/pacemaker/cib")
        mock_isdir.return_value = True
        mock_exists.return_value = True
        mock_corosync_get_value.return_value = "/var/lib/cluster/corosync.log"

        core.load_context_attributes(mock_ctx_inst)

        self.assertEqual(mock_ctx_inst.pcmk_lib_dir, "/var/lib/pacemaker")
        self.assertEqual(mock_ctx_inst.cores_dir_list, ["/var/lib/pacemaker/cores", constants.COROSYNC_LIB])
        self.assertEqual(mock_ctx_inst.corosync_logfile, mock_corosync_get_value.return_value)
    ...
    @mock.patch('crmsh.utils.check_space_option_value')
    @mock.patch('crmsh.report.core.add_arguments')
    def test_parse_arguments(self, mock_parse, mock_check_space, mock_process):
    ...
    def test_add_arguments(self, mock_argparse, mock_formatter, mock_config):
        ...
        core.add_arguments()
        self.assertEqual(mock_config.report.verbosity, True)

    @mock.patch('logging.Logger.debug')
    @mock.patch('crmsh.utils.to_ascii')
    @mock.patch('crmsh.utils.get_stdout_stderr')
    @mock.patch('crmsh.utils.this_node')
    def test_push_data(self, mock_this_node, mock_run, mock_to_ascii, mock_debug):
        mock_this_node.return_value = "node1"
        mock_run.return_value = (0, "data", "error")
        mock_to_ascii.return_value = "error"
        mock_ctx_inst = mock.Mock(work_dir="/opt/work_dir", main_node="node1")
        ...
        mock_run.assert_called_once_with("cd /opt/work_dir/.. && tar -h -cf - node1", raw=True)
    ...
    def test_process_arguments_error(self, mock_list_nodes):
        mock_ctx_inst = mock.Mock(from_time=123, to_time=150, node_list=[])
    ...
    @mock.patch('crmsh.utils.this_node')
    def test_process_arguments(self, mock_this_node, mock_dest):
        mock_ctx_inst = mock.Mock(from_time=123, to_time=150, node_list=["node1"], single=True)
