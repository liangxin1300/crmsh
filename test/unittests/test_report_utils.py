...
class TestPackage(unittest.TestCase):
    ...
    def setUp(self, mock_get_pkg_mgr):
        ...
        self.inst_deb = utils.Package("deb1")
    ...
    def test_version_deb(self):
        self.inst_deb.pkg_ver_deb()
    ...
    @mock.patch('crmsh.report.utils.Package.version')
    def test_verify(self, mock_version, mock_verify_rpm):
    ...
    def test_verify_rpm(self, mock_run):
        self.inst.installed = "rpm1 rpm2"
        mock_run.side_effect = [(0, ""), (1, "error")]
        res = self.inst.verify_rpm()
        self.assertEqual(res, "For package rpm2:\nerror\n")

    def test_verify_deb(self):
        self.inst_deb.verify_deb()
...
class TestSanitizer(unittest.TestCase):
    ...
    def test_load_cib_from_work_dir_no_cib(self, mock_this_node, mock_exists):
        ...
        mock_exists.assert_called_once_with(f"/opt/node1/{constants.CIB_F}")

    @mock.patch('crmsh.report.utils.read_from_file')
    @mock.patch('os.path.exists')
    @mock.patch('crmsh.utils.this_node')
    def test_load_cib_from_work_dir_empty(self, mock_this_node, mock_exists, mock_read):
        mock_this_node.return_value = "node1"
    ...
    @mock.patch('os.path.exists')
    @mock.patch('crmsh.utils.this_node')
    def test_load_cib_from_work_dir(self, mock_this_node, mock_exists, mock_read):
        mock_this_node.return_value = "node1"
        mock_exists.return_value = True
...
class TestUtils(unittest.TestCase):
    ...
    def test_get_cmd_output(self, mock_run):
        mock_run.return_value = (0, "stdout_data", "stderr_data")
        res = utils.get_cmd_output("cmd")
        self.assertEqual(res, "stdout_data\nstderr_data\n")
        mock_run.assert_called_once_with("cmd", timeout=None)
    ...
    def test_read_from_file_none(self, mock_get_open_method, mock_error):
        mock_open_instance = mock_get_open_method.return_value
        mock_open_instance.side_effect = Exception("File not found")
        res = utils.read_from_file('noexistent.txt')
        self.assertIsNone(res)
        mock_error.assert_called_once_with('When reading file "%s": %s', 'noexistent.txt', 'File not found')

    @mock.patch('crmsh.utils.to_ascii')
    @mock.patch('crmsh.report.utils.get_open_method')
    def test_read_from_file_success(self, mock_get_open_method, mock_to_ascii):
        mock_open_instance = mock_get_open_method.return_value
        mock_open_instance.return_value.__enter__.return_value.read.return_value = "Test content"
        mock_to_ascii.return_value = "Test content (ascii)"

        result = utils.read_from_file('testfile.txt')

        self.assertEqual(result, "Test content (ascii)")

    @mock.patch('bz2.open')
    def test_get_open_method(self, mock_bz2_open):
        res = utils.get_open_method("file.bz2")
        self.assertEqual(res, mock_bz2_open)
    ...
    def test_dump_logset_return(self, mock_arch):
    ...
    @mock.patch('crmsh.report.utils.arch_logs')
    def test_dump_logset_irrgular(self, mock_arch, mock_print, mock_basename, mock_str2file, mock_debug2):
        ...
        mock_debug2.assert_called_once_with("Dump file1 into /opt/work_dir/file1")
    ...
    @mock.patch('crmsh.report.utils.print_logseg')
    @mock.patch('crmsh.report.utils.arch_logs')
    def test_dump_logset_one(self, mock_arch, mock_print, mock_basename, mock_str2file, mock_debug2):
    ...
    @mock.patch('crmsh.report.utils.print_logseg')
    @mock.patch('crmsh.report.utils.arch_logs')
    def test_dump_logset(self, mock_arch, mock_print, mock_basename, mock_str2file, mock_debug2):
...
ANSI_COLOR="0;32"
    ...
    @mock.patch('os.path.exists')
    @mock.patch('crmsh.report.utils.logger.debug2')
    def test_get_distro_info_lsb(self, mock_debug2, mock_exists, mock_run, mock_which):
        mock_which.return_value = True
        mock_exists.return_value = False
...
line2
    ...
    @mock.patch('crmsh.utils.parse_time')
    @mock.patch('crmsh.report.utils.head')
    def test_determin_log_format_rfc5424(self, mock_head, mock_parse):
        mock_head.return_value = ["2003-10-11T22:14:15.003Z mymachine.example.com su"]
...
2003-10-11T22:14:15.003Z mymachine.example.com su
    ...
    @mock.patch('crmsh.utils.parse_time')
    @mock.patch('crmsh.report.utils.head')
    def test_determin_log_format_syslog(self, mock_head, mock_parse):
        mock_head.return_value = ["Feb 12 18:30:08 15sp1-1 kernel:"]
        mock_parse.side_effect = [None, 123456]
        data = """
Feb 12 18:30:08 15sp1-1 kernel:
        ...
        mock_parse.assert_has_calls([
            mock.call("Feb", quiet=True),
            mock.call("Feb 12 18:30:08", quiet=True)
        ])
    ...
    def test_determin_log_format_legacy(self, mock_head, mock_parse):
        ...
        data = """
Legacy 003-10-11T22:14:15.003Z log
        """
        res = utils.determin_log_format(data)
    ...
    def test_cib_diff(self, mock_isfile, mock_run):
        ...
        self.assertEqual(rc, 0)
        self.assertEqual(out, "")
        mock_run.assert_called_once_with("crm_diff -c -n /opt/node1/cib.xml -o /opt/node2/cib.xml")
    ...
    def test_ts_to_dt(self):
        res = utils.ts_to_dt(1693519260.0)
        self.assertEqual(utils.dt_to_str(res), "2023-09-01 06:01")
    ...
    def test_parse_to_timestamp_none(self, mock_parse, mock_error):
        ...
        res = utils.parse_to_timestamp("xxxxx")
        self.assertIsNone(res)
...
line4
        ...
        expected_data = """
WARNINGS and ERRORS in /opt/workdir/pacemaker.log:
pacemaker-controld[5678]:  warning: data
pacemaker-schedulerd[5677]:  error: Resource"""
