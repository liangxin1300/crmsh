...
# Set the permissions on the above log file to owner/group read/write
    ...
    def test_collect_ha_logs(self, mock_get_log, mock_isfile, mock_dump, mock_get_value):
        ...
        mock_dump.assert_has_calls([
            ...
            mock.call(mock_ctx_inst, mock_get_value.return_value)
            ])
    ...
    def test_collect_journal_logs(self, mock_ts_to_str, mock_debug, mock_get_cmd_output,
        ...
        mock_debug.assert_called_once_with("Collect journal logs since: 10.10 until: 10.12")
        cmd_list = [
        ...
        mock_debug2.assert_has_calls([
            ...
            mock.call(f"Dump jounal log for default into /opt/work/{constants.JOURNAL_F}"),
            mock.call(f"Running command: {cmd_list[1]}"),
            mock.call(f"Dump jounal log for pacemaker into /opt/work/{constants.JOURNAL_PCMK_F}"),
            mock.call(f"Running command: {cmd_list[2]}"),
            mock.call(f"Dump jounal log for corosync into /opt/work/{constants.JOURNAL_COROSYNC_F}"),
            mock.call(f"Running command: {cmd_list[3]}"),
            mock.call(f"Dump jounal log for sbd into /opt/work/{constants.JOURNAL_SBD_F}")
            ])
    ...
    def test_collect_sbd_info_no_config(self, mock_exists, mock_debug2):
        mock_exists.return_value = False
        mock_ctx_inst = mock.Mock()
        collect.collect_sbd_info(mock_ctx_inst)
        mock_exists.assert_called_once_with(constants.SBDCONF)
        mock_debug2.assert_called_once_with(f"SBD config file {constants.SBDCONF} does not exist")
    ...
    @mock.patch('shutil.copy2')
    @mock.patch('os.path.exists')
    def test_collect_sbd_info(self, mock_exists, mock_copy, mock_which, mock_run, mock_debug2, mock_open_file):
        mock_exists.return_value = True
        ...
        mock_debug2.assert_called_once_with(f"Dump SBD config file into {mock_ctx_inst.work_dir}/{constants.SBD_F}")
    ...
    def test_pe_to_dot(self, mock_run, mock_warning):
        ...
        mock_run.assert_called_once_with("crm_simulate -D /opt/pe-input-0.dot -x /opt/pe-input-0.bz2")
    ...
    @mock.patch('crmsh.report.utils.find_files_in_timespan')
    @mock.patch('logging.Logger.debug')
    def test_collect_pe_inputs(self, mock_debug, mock_find_files, mock_mkdir, mock_symlink, mock_to_dot):
        ...
        mock_debug.assert_has_calls([
            ...
            mock.call(f"Dump PE files into {mock_ctx_inst.work_dir}/pe_dir")
            ])
    ...
    @mock.patch('logging.Logger.warning')
    @mock.patch('crmsh.report.utils.get_cmd_output')
    def test_collect_sys_stats(self, mock_run, mock_warning, mock_str2file, mock_debug2):
        mock_run.side_effect = [
    ...
    def test_collect_sys_info(self, mock_package, mock_uname, mock_str2file, mock_get_distro, mock_debug2):
        ...
        mock_str2file.assert_called_once_with('##### Installed cluster related packages #####\nversion_data\nverify_data\n\n\n##### System info #####\nPlatform: Linux\nKernel release: 4.5\nArchitecture: x86_64\nDistribution: suse\n', '/opt/work/sysinfo.txt')
        mock_debug2.assert_called_once_with(f"Dump packages and platform info into {mock_ctx_inst.work_dir}/{constants.SYSINFO_F}")
    ...
    def test_collect_config_running(self, mock_active, mock_dump_state, mock_write, mock_debug2, mock_cib, mock_dump_config):
    ...
    @mock.patch('shutil.copy2')
    @mock.patch('crmsh.utils.service_is_active')
    def test_collect_config_stopped(self, mock_active, mock_copy2, mock_write, mock_debug2, mock_cib, mock_dump_config):
    ...
    def test_consume_cib_in_workdir(self, mock_isfile, mock_run, mock_str2file):
        mock_isfile.return_value = True
        mock_run.side_effect = ["data1", "data2"]
    ...
    @mock.patch('logging.Logger.debug')
    @mock.patch('crmsh.utils.get_stdout_or_raise_error')
    def test_collect_ratraces(self, mock_run, mock_debug, mock_find, mock_mkdirp, mock_copy, mock_debug2):
        mock_run.return_value = "INFO: Trace for .* is written to /var/log/cluster/pacemaker.log"
        mock_ctx_inst = mock.Mock(node_list=["node1"], work_dir="/opt/work")
        mock_find.return_value = ["/var/log/cluster"]

        collect.collect_ratraces(mock_ctx_inst)

        mock_debug.assert_called_once_with('Looking for RA trace files in "%s"', '/var/log/cluster')
        mock_debug2.assert_called_once_with('Dump RA trace files into /opt/work/var/log')
...
/dev/sda7 on /srv/clusterfs type ocfs2 (rw,relatime,heartbeat=non
        """
        mock_run.side_effect = [(0, mount_data, None), (0, "data", None)]
    ...
    @mock.patch('crmsh.utils.str2file')
    @mock.patch('crmsh.utils.get_stdout_stderr')
    def test_collect_ocfs2_info(self, mock_run, mock_str2file, mock_debug2, mock_D, mock_lsof, mock_output):
    ...
    @mock.patch('crmsh.report.utils.get_cmd_output')
    @mock.patch('shutil.which')
    def test_collect_dlm_info(self, mock_which, mock_get_output, mock_str2file, mock_debug2):
        mock_which.return_value = True
        ls_data = """
dlm lockspaces
...
id            0x19041a12
        ...
        mock_debug2.assert_called_once_with(f"Dump DLM information into /opt/work_dir/{constants.DLM_DUMP_F}")
    ...
    def test_find_binary_path_for_core_not_found(self, mock_run):
        mock_run.return_value = (0, "Core not found", None)
    ...
    def test_find_binary_path_for_core(self, mock_run):
        mock_run.return_value = (0, "Core was generated by `/usr/sbin/crm_mon'", None)
        res = collect.find_binary_path_for_core("core.1")
    ...
    @mock.patch('logging.Logger.warning')
    @mock.patch('shutil.which')
    def test_dump_core_info_no_gdb(self, mock_which, mock_warning, mock_str2file, mock_debug2):
        mock_which.return_value = False
        ...
        mock_debug2(f"Dump coredump info into /opt/workdir/{constants.COREDUMP_F}")

    @mock.patch('crmsh.report.collect.logger.debug2')
    @mock.patch('crmsh.utils.str2file')
    @mock.patch('crmsh.report.collect.find_binary_path_for_core')
    @mock.patch('shutil.which')
    def test_dump_core_info(self, mock_which, mock_find_binary, mock_str2file, mock_debug2):
        mock_which.return_value = True
    ...
    @mock.patch('crmsh.utils.str2file')
    @mock.patch('crmsh.utils.get_stdout_or_raise_error')
    def test_dump_runtime_state(self, mock_run, mock_str2file, mock_debug2, mock_get_dc, mock_this_node):
        mock_run.side_effect = ["crm_mon_data", "cib_data", "crm_node_data"]
        mock_get_dc.return_value = "node1"
        mock_this_node.return_value = "node1"
        collect.dump_runtime_state("/opt/workdir")
        mock_debug2.assert_has_calls([
            mock.call(f"Dump cluster state into /opt/workdir/{constants.CRM_MON_F}"),
            mock.call(f"Dump CIB contents into /opt/workdir/{constants.CIB_F}"),
            mock.call(f"Dump members of this partition into /opt/workdir/{constants.MEMBERSHIP_F}"),
            mock.call(f"Current DC is node1; Touch file 'DC' in /opt/workdir")
        ])
    ...
    @mock.patch('crmsh.report.utils.get_cmd_output')
    @mock.patch('crmsh.report.utils.find_files_in_timespan')
    def test_collect_corosync_blackbox(self, mock_find_files, mock_get_cmd_output, mock_str2file, mock_debug2):
        mock_ctx_inst = mock.Mock(work_dir="/opt/workdir")
        ...
        mock_debug2.assert_called_once_with(f"Dump corosync blackbox info into /opt/workdir/{constants.COROSYNC_RECORDER_F}")
