...
[[cmdhelp_insight,Cluster insight]]
=== `insight` - Cluster insight

Cluster insight help details

[[cmdhelp_insight_preflight_check,Cluster preflight check tool set]]
==== `preflight_check` - Cluster preflight check tool set

To trigger cluster failures and verify some key configuration 
right before it goes to production. Cluster failure simulation 
by different people can go wild and the results can be different, 
even wrongly interpreted. This tool standardizes the steps as the simple CLI. 
It is carefully designed and must not change any configuration to harm the production.
...
optional arguments:
  -c, --check-conf        Valid the configurations
