...
# crm configure<<EOF
  ...
  location node_pref internal_www \
    rule score=50 expression attribute=#uname operation=eq value=node1 \
    rule score-attribute=pingd expression attribute=pingd operation=defined
  #
  # cluster properties
  #
...
...............
crm(live)configure# location websvc-pref websvc on xen-b score=100
...............
...
location websvc-pref websvc on xen-b score=100
...............
...
primitive my_resource Special \
  ...
  extra score=2 rule expression attribute=#uname operation=eq value=node1 \
...
..............
rules ::
  rule [id_spec] [rule_attributes] <expression>
  [rule [id_spec] [rule_attributes] <expression> ...]

id_spec :: $id=<id> | $id-ref=<id>
rule_attributes :: <score> | <score-attribute> | <role>
score :: <number> | <attribute> | [-]inf
expression :: <simple_exp> [<bool_op> <simple_exp> ...]
bool_op :: or | and
simple_exp :: <attribute> <binary_op> <value>
          | <attribute> <unary_op>
          | date <date_expr>
...
Usage:
...............
colocation <id> <rsc> [<role>] with <with-rsc> [<with-role>] [options]

colocation <id> <resource_sets> [options]

options :: <node-attribute>|<score>

resource_sets :: resource_set [resource_set ...]

resource_set :: [<rsc> [<rsc> ...] [attributes]

attributes :: [require-all=(true|false)] [sequential=(true|false)]
...............
Example:
...............
colocation never_put_apache_with_dummy apache with dummy options score=-inf
colocation c1 \
    resource_set A \
...
* Plain resource reference: +location loc1 webserver on node1 score=100+
* Resource set: +location loc1 resource_set virtual-ip webserver on node1 score=100+
* Tag containing resource ids: +location loc1 tag1 on node1 score=100+
* Resource pattern: +location loc1 rsc-pattern=/web.*/ on node1 score=100+
...
Usage:
...............
location <id> <rsc> {<node_pref>|<rules>} [<options>]

rsc :: /<rsc-pattern>/
        | <resource_sets>
        | <rsc>

node_pref :: on <node> <score>

options :: role=<role> | resource-discovery=always|never|exclusive
...
Examples:
...............
location conn_1 internal_www on node1 score=100
...
location conn_2 dummy_float \
  ...
  expression attribute=pingd operation=lte value=0 type=number

# never probe for rsc1 on node1
location no-probe rsc1 on node1 score=-inf options resource-discovery=never
...
location l5 m5 \
        ...
        date operation=in_range start="2009-05-26" end="2009-07-26"
...............
...
Usage:
...............
order <id> first then [options]

order <id> resource_sets [options]

first :: first <rsc> [<first-action>]

then ::  then <rsc> [<action>]

options :: <kind>|<require-all><score>|<symmetrical>
...
Example:
...............
order o-1 first apache first-action=start then ip_1 options kind=Mandatory
order o-2 \
    resource_set A \
    resource_set B C sequential=false \
    options kind=Serialize
order o-3 \
    resource_set A B require-all=false sequential=false \
    resource_set C \
    options score=inf
order o-4 first first-resource then then-resource
...............
...
primitive mySpecialRsc Special \
  params interface=eth1 \
  extra score=3 rule expression attribute=#uname operation=eq value=node1 \
  params interface=eth2 port=8888 \
  extra score=2 rule expression attribute=#uname operation=eq value=node2 \
  params interface=eth0 port=9999 \
  extra score=1
...
Usage:
...............
rsc_ticket <id> <ticket_id> <rsc> [<role>] [options]

rsc_ticket <id> <ticket_id> <resource_sets> [options]

options :: loss_policy_action
...
Example:
...............
rsc_ticket ticket-A_public-ip ticket=ticket-A public-ip
rsc_ticket ticket-A_bigdb ticket=ticket-A bigdb options loss-policy=fence
