...
from crmsh import config, log, userdir, corosync, tmpfiles
...
def add_arguments() -> argparse.Namespace:
    ...
    config.report.verbosity = args.debug
...
def push_data(context: Context) -> None:
    """
    """
    logger.debug(f"Pushing data from {crmutils.this_node()}:{context.work_dir} to {context.main_node}")
    cmd = f'cd {context.work_dir}/.. && tar -h -cf - {crmutils.this_node()}'
    _, out, err = crmutils.get_stdout_stderr(cmd, raw=True)
    if out:
...
def pick_compress_prog(context: Context) -> None:
    ...
    for cmd, suffix in compress_prog_suffix_dict.items():
        if shutil.which(cmd):
            context.compress_prog, context.compress_suffix = cmd, suffix
            return
...
def process_results(context: Context) -> None:
    ...
    if context.no_compress:
        ...
        crmutils.get_stdout_or_raise_error(cmd)
...
def start_collector(node: str, context: Context) -> None:
    ...
    if node == crmutils.this_node():
        ...
        code, out, err = crmutils.su_get_stdout_stderr(context.ssh_user, cmd)
        if code != 0:
            pass
        if err:
            print(err)
    ...
    cmd = f"(cd {context.work_dir} && tar xf -)"
...
def process_arguments(context: Context) -> None:
    ...
    if context.single:
        context.node_list = [crmutils.this_node()]
    process_dest(context)
    context.main_node = crmutils.this_node()
...
def load_from_corosync_conf(context: Context) -> None:
    if not os.path.exists(corosync.conf()):
        logger.warning(f"File {corosync.conf()} does not exist")
        return
    context.corosync_logfile = corosync.get_value('logging.logfile')
...
def setup_workdir(context: Context) -> None:
    ...
    if not is_collector():
        ...
        context.work_dir = os.path.join(tmpdir,
                                        ...
                                        crmutils.this_node())
    tmpfiles._mkdir(context.work_dir)
    logger.debug(f"Setup work directory in {context.work_dir}")
