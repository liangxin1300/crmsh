...
from crmsh import config, log, userdir, corosync, tmpfiles, ui_cluster
...
def push_data(context: Context) -> None:
    ...
    """
    this_node = crmutils.this_node()
    logger.debug(f"Pushing data from {this_node}:{context.work_dir} to {context.main_node}")
    cmd = f'cd {context.work_dir}/.. && tar -h -cf - {this_node}'
