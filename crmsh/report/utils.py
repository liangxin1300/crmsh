...
from crmsh import corosync, log, userdir, tmpfiles, config
...
def cib_diff(file1: str, file2: str) -> Tuple[int, str]:
    ...
    if (os.path.isfile(os.path.join(node1_dir, "RUNNING")) and
        ...
        code, out_string, _ = crmutils.get_stdout_stderr(cmd)
    else:
...
class Sanitizer:
    ...
    def _load_cib_from_work_dir(self) -> None:
        ...
        if not data:
            raise ReportGenericError(f"File {cib_file} is empty")
        self.cib_data = data
