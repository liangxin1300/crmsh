...
class Context(object):
    ...
    def _validate_sbd_option(self):
        ...
        if self.sbd_devices and self.diskless_sbd:
            fatal("Can't use -s and -S options together")
        if self.stage == "sbd":
            if not self.sbd_devices and not self.diskless_sbd and self.yes_to_all:
                fatal("Stage sbd should specify sbd device by -s or diskless sbd by -S option")
            if utils.service_is_active("sbd.service"):
                fatal("Cannot configure stage sbd: sbd.service already running!")
...
def invoke(*args):
    ...
    if stdout:
        logger_utils.log_only_to_file("invoke stdout: {}".format(stdout))
    if stderr:
        logger_utils.log_only_to_file("invoke stderr: {}".format(stderr))
...
def change_user_shell(user):
    ...
    if user != "root" and is_nologin(user):
        if not _context.yes_to_all:
            logger.info("""User {} will be changed the login shell as /bin/bash, and
...
def configure_qdevice_interactive():
    ...
    if not qnetd_addr:
        fatal("Address of QNetd is required")
...
def remove_node_from_cluster():
    ...
    if not rc:
        fatal("Failed to remove {}: {}".format(node, err))
