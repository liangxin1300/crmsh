...
def add_node(addr, name=None):
    ...
    if addr in nodenames + coronodes or (ipaddr and ipaddr in coronodes):
        logger.warning.warning("%s already in corosync.conf" % (addr))
        return
    if name and name in nodenames + coronodes:
        logger.warning.warning("%s already in corosync.conf" % (name))
        return
    if addr in nodes:
        logger.warning.warning("%s already in configuration" % (addr))
        return
    if name and name in nodes:
        logger.warning.warning("%s already in configuration" % (name))
        return
